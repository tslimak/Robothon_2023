...
def segment_arb_pts(seg,
    ...
    if not isinstance(seg, Segment):
        ...
        if isinstance(seg, np.ndarray):
            ...
            raise TypeError(f'seg is of type: {type(seg)}, but it needs to be either type Segment, np.ndarray, or list')  # noqa: E501
...
def shrink_bbox(bbox_cor, shrink_val: int = 0.95) -> np.ndarray:
    ...
    if not isinstance(bbox_cor, np.ndarray):
        ...
        if isinstance(bbox_cor, list):
            ...
            raise TypeError(f'bbox_cor needs to be the type of either list or np.ndarray not {type(bbox_cor)}')  # noqa: E501
