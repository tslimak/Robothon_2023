...
def col_thr(img, col_space=0, channel=3, thr_perc=0.7, erode=0):
  ...
  #ch = cv2.cvtColor(col_img, cv2.COLOR_BGR2GRAY)
  #cv2.imshow('Gray Image', ch)
...
def shape_detect(img, thr_img, vert_p = 3, perc=[0.15], min_area=30, max_area=100):
  ...
  #cv2.imshow('shape_detect/thr_img', thr_img)
...
def crop_img(img, shape_cont):
  ...
  if len(shape_cont) == 1:
    ...
    cp_img = img[min_vertex[0][1]:max_vertex[0][1], min_vertex[0][0]:max_vertex[0][0]]
    #cv2_imshow(cp_img)
...
def calc_dist(crop_img, tri_cont, req):

  print('number of triangles: ' + str(len(tri_cont)))

  # get top & bottom vertex of each triangle
  top_vert = []
  bot_vert = []

  if len(tri_cont) == 2 or len(tri_cont) == 1:
    for tri in tri_cont:
  ...
  if req == 0:
    ...
    # calculate distance between vertex points
    top_dist = np.subtract(np.array(top_vert[0]), np.array([9, 50]))
    print('top vertex distance: ' + str(top_dist))

    bot_dist = np.subtract(np.array(bot_vert[0]), np.array([20, 50]))
    print('bottom vertex distance: ' + str(bot_dist))
...
def handle_triangle_detection(req):
  ...
  print('before wait message')
  if req.triangleID == 0:
    sensor_image1 = rospy.wait_for_message('/camera/color/image_raw', Image)
    sensor_image = sensor_image1
  #image = np.frombuffer(sensor_image.data, dtype=np.uint8).reshape(sensor_image.height, sensor_image.width, -1)
  if req.triangleID == 1:
    sensor_image2 = rospy.wait_for_message('/camera/color/image_raw', Image)
    sensor_image = sensor_image2

  print('after wait message')


  ###################################################################

  #rospy.loginfo(sensor_image.header)

  # Try to convert the ROS Image message to a CV2 Image
  try:
    bridge = CvBridge()
    image = bridge.imgmsg_to_cv2(sensor_image, "passthrough")
  except CvBridgeError as e:
    rospy.logerr("CvBridge Error: {0}".format(e))

  #cv2.imshow("Camera Image First Try", image)
  ...
  shape_cont = shape_detect(image, thr_img, vert_p=4, perc=[0.1], min_area=4000, max_area=12000)
  # crop image -> image = only display
  cp_img = crop_img(image, shape_cont)

  #cv2.imshow('cropped image', cp_img)

  print('search for triangles')

  ## detect triangle

  # find triangle
  if req.triangleID == 0:
    thr_img = col_thr(cp_img, col_space=0, channel=0, thr_perc=0.7, erode=0) # erode=3)
    #shape_cont = shape_detect(cp_img, thr_img, vert_p=3, perc=[0.2], min_area=50, max_area=110)
    shape_cont = shape_detect(cp_img, thr_img, vert_p=3, perc=[0.17], min_area=50, max_area=110)
  elif req.triangleID == 1:
    thr_img = col_thr(cp_img, col_space=0, channel=0, thr_perc=0.7, erode=0) # erode=3)
    #shape_cont = shape_detect(cp_img, thr_img, vert_p=3, perc=[0.2], min_area=35, max_area=110) #worked mostly well
    shape_cont = shape_detect(cp_img, thr_img, vert_p=3, perc=[0.17], min_area=30, max_area=110)
  ...
  dist = calc_dist(cp_img, shape_cont, req.triangleID)
